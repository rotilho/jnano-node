...
internal class ReceiveBlockCodecTest {
    private val node = Node(InetSocketAddress(InetAddress.getLocalHost(), 7050), Environment.LIVE, 13)
    private val codec = TCPCodec(PacketType.CONFIRM_REQUEST, node, TransactionCodec(ReceiveBlockCodec()))

    @Test
    fun `Should decode and encode confirm request`() {
        val encoded =
